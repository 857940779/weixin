...
(function($ , undefined) {
	...
	function Sidebar(sidebar, settings) {
		...
		this.$sidebar.on(ace.click_event+'.ace.submenu', '.nav-list', function (ev) {
			...
			//面包屑HTML: 默认有：工作台
			var liHtml = '<li><i class="ace-icon fa fa-home home-icon"></i><a href="/admin/index">工作台</a></li>';
			$.each(breadcrumb_items,function(n,menuItem) {  
		      		if(n == breadcrumb_items.length-1){
		      		    ...
		      		    if(menuItem.href){
		      		    	liHtml += "<li><a href'"+menuItem.href+"'>"+menuItem.text+"</a></li>"; //存在href，提供链接
		      		    }else{
		  	 ...
		  	 //获取当前选中菜单的href属性，如果href为空，则不更新面包屑内容(比如：点击父菜单，不需要更新)
		   	 var activeMenuHref = link_element.attr("href");
		   	 if(activeMenuHref && activeMenuHref!="javascript:void(0)"){
